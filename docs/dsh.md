...
caddy in front of `dsh-web`, holding a self-signed certificate for that address, so your
browser will ask you to accept it. Do — see the warning below for why this is not
optional.
...
> from scratch each boot, and the certificate with it. Clicking through still gives you
> an `https` origin, which is all the page needs.
>
> It is a bare self-signed leaf — there is no certificate authority anywhere in this, and
> nothing is ever added to a trust store on the guest or on your host. It is generated by
> the caddy service's own first act, as the unprivileged `caddy` user, inside the state
> directory systemd hands it. Keeping it between boots would mean storing guest state on
> your host, and baking it into the Nix store would publish the private key to anyone who
> can read `/nix/store`; neither is worth trading for one click.

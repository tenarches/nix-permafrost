...
let
  ...
  #
  # Emitted on stdout rather than installed: the host's ~/.ssh/config.d is
  # deliberately hand-managed, so this stays a file the operator places, and
  # nothing here reaches into the host.
  sshConfigScript = pkgs.writeShellScriptBin "permafrost-ssh-config" ''
    cat <<'EOF'
    # Generated by `nix run .#ssh-config` from nix-permafrost modules/inventory.nix.
    # Regenerate after changing the inventory rather than editing this file.
    #
    # PERMAFROST_AGENT_SOCK must be exported in the session you run ssh from,
    # holding the path of the permafrost agent socket — the one carrying the
    # agentic key alone. Point it at the personal agent and every guest gains
    # the use of your personal key. If it is unset, agent forwarding does not
    # happen; `ssh -G` cannot detect that, because it prints the variable name
    # without resolving it. Check inside a guest with `ssh-add -l` instead.

    Host 192.168.33.* permafrost-*
      User agent
      ForwardAgent $PERMAFROST_AGENT_SOCK

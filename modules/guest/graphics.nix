{
  flake.modules.nixos.guest-graphics =
    {
      pkgs,
      lib,
      config,
    ...
    # rather than on permafrost.gui directly, so it self-activates wherever the
    # host side turns graphics on — which today is only guest/launch-runner.nix,
    # for the reason given in guest/launch-fleet.nix.
    #
    ...
    lib.mkIf config.microvm.graphics.enable {
      ...
      #
      # Everything else — the toolkit hints — lives in guest/base.nix, because
